export default async function handler(req, res) {
  ...
  if (error) return res.redirect(`/?auth=error&msg=${encodeURIComponent(error_description||error)}`);
  ...
  try {
    ...
    const accounts = (d3.data||[]).map(a => ({ id: a.id.replace('act_',''), name: a.name, currency: a.currency }));
 
    const params = new URLSearchParams({ auth:'success', platform:'meta', token, accounts: JSON.stringify(accounts) });
